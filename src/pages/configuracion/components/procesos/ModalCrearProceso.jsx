...
import { crearProceso } from '../../services/procesoServices'
import { useParams } from 'react-router-dom'

const ModalCrearProceso = (props) => {
    const {register, handleSubmit, setError, formState: { errors }, setValue, reset} = useForm({ mode: "onChange"});
    const [messageError, setMessageError] = useState(false);
    const [loading, setLoading] = useState(false);
    const {cerrarModal, setProcesos} = props;
    ...
    const onSubmit = async(values) => {
        ...
        try {
            ...
            const { data } = result 
            setProcesos(prevGrupos => [data, ...prevGrupos]);
            cerrarModal();
            setValue("nombre", "");
            toast.success(`El proceso ha sido creado exitosamente.`);
            reset();
        } catch (error) {
...
export default ModalCrearProceso